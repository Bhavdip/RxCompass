...
public class CompassView extends View {
  ...
  private Canvas init(Canvas canvas) {
    ...
    for (int i = 10; i <= 360; i += 10) {
      ...
      canvas.drawText("" + i, this.px, textSize + this.textScaleStyle.getTextSize(),
          this.textScaleStyle);
    ...
    float textSize2 =
        ((-this.padding) + this.textScaleStyle.getTextSize()) + (resources.getDimension(
  ...
  private void init() {
    ...
    this.textWhiteColor.setTextAlign(Paint.Align.CENTER);

    this.textScaleStyle = new Paint(this.textWhiteColor);

    this.countryNeedleStyle = new Paint(1);
    this.countryNeedleStyle.setStyle(Paint.Style.FILL);
    ...
    linePaint.setStyle(Paint.Style.FILL); // We will change it at the end.
  ...
  private Canvas calculateTextSize(Canvas canvas) {
    ...
    this.paintBaseColor.setStyle(Paint.Style.STROKE);
  ...
  private void calculateTextSize() {
    ...
    this.textRedColor.setTextSize(
        resources.getDimension(R.dimen.compass_main_fontsize) * this.radius);
    this.textWhiteColor.setTextSize(
        resources.getDimension(R.dimen.compass_main_fontsize) * this.radius);
    this.textScaleStyle.setTextSize(
        resources.getDimension(R.dimen.compass_sub_fontsize) * this.radius);
    this.dotCirceStyle.setStrokeWidth(
        resources.getDimension(R.dimen.compass_dot_radius) * this.radius);
    calcNeedlePosition();
  }
  ...
  private void calculateDegree(Canvas canvas) {
    canvas.drawCircle(this.px, this.py,
        getResources().getDimension(R.dimen.compass_center_radius) * this.radius,
        this.paintBaseColor);
  }
